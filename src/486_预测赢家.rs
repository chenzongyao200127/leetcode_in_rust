...
impl Solution {
    pub fn predict_the_winner(nums: Vec<i32>) -> bool {
        #[inline]
        fn dfs(l: usize, r: usize, nums: &Vec<i32>) -> i32 {
            if l == r {
                return nums[l];
            }

            let pick_frist = nums[l] - dfs(l + 1, r, nums);
            let pick_last = nums[r] - dfs(l, r - 1, nums);

            pick_frist.max(pick_last)
        }

        dfs(0, nums.len()-1, &nums) >= 0
    }
}
